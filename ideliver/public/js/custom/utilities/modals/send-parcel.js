			...
			form,
			{
				fields: {
					...
					billing_order_postcode: {
						validators: {
							notEmpty: {
								message: 'Pickup postal code is required'
							}
						}
					},
					billing_order_suburb: {
						validators: {
							notEmpty: {
								message: 'Pickup suburb is required'
							}
						}
					}
				},
		...
		validations.push(FormValidation.formValidation(
			form,
			{
				fields: {
					...
					receiver_order_suburb: {
						validators: {
							notEmpty: {
								message: 'Receiver suburb is required'
							}
						}
					}
				},
...
$('#addParcel').click(function(e) {
    ...
    if (isEmpty($.session.get('parcels'))==true){

        var parcelsG = [];

        parcelsG = insertArrayAt(parcelsG,1,parcels);
    ...
    $.session.set('parcels',JSON.stringify(parcelsG));

    $("#itemsG").html( '<input class="form-control" name="items"  value="'+btoa($.session.get('parcels'))+'" />');

    console.log(parcelsG)
});
// On document ready
